...
function MagusName($timeout) {
    return{
        restrict: 'E',
        template: '<input placeholder="{{placeholder}}" class="form-control" ng-class="required && (value.length <= 2 || !value) ? \'input-error\' : \'\'" required="{{required}}" ng-model="value" name="value" /> ',
        require: '?ngModel',
        scope: {
            required: '=',
            placeholder: '@'
